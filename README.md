...
- The `GlobalLikelihood` object has a `sample_conditional(key, params) -> (key, dict)` property–a JAX-ready function that returns a dictionary of GP coefficients drawn from the conditional (normal) probability distribution of the `globalgp`(s), given the `params`. This function calls `GlobalLikelihood.conditional(params)`, which returns a concatenated vector `mu` of the mean GP coefficients and the result `cf` of running `scipy.linalg.cho_factor` on the matrix `Phi^-1 + T^t K T`, where `Phi` and `T` are prior and basis of `globalgp`.
